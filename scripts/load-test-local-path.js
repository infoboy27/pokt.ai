// k6 Load Test Script for Local PATH Gateway - Ethereum (ETH) Only
// Target: http://localhost:3069/v1/rpc with Target-Service-Id: eth header
...
const GATEWAY_URL = __ENV.GATEWAY_URL || 'http://localhost:3069/v1/rpc';

// Chain configuration - ETH only
const CHAINS = [
  { serviceId: 'eth', method: 'eth_blockNumber', weight: 1 }
];
...
function generateHTMLReport(data, chainStats) {
  ...
  const chainRows = CHAINS.map(chain => {
    const chainData = chainStats[chain.serviceId] || { requests: 0, percentage: '0%' };
...
<head>
    ...
    <title>Local PATH Gateway Load Test Report - pokt.ai</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            ...
            <h1>Local PATH Gateway Load Test Report</h1>
            <div class="subtitle">${TOTAL_REQUESTS.toLocaleString()} Requests @ ${TARGET_RPS.toLocaleString()} RPS - Ethereum (ETH)</div>
        ...
        <div class="content">
            <div class="config-info">
                ...
                <div class="info-grid">
                    ...
                    <div class="info-row"><div class="info-label">Chain</div><div class="info-value">Ethereum (ETH)</div></div>
                </div>
            </div>
            <div class="summary">
            ...
            <div class="section">
                ...
                <table class="table">
                    <thead><tr><th>Chain</th><th>Requests</th><th>Percentage</th><th>Status</th></tr></thead>
...
export function handleSummary(data) {
  ...
  const summaryFile = `load-test-results/local-path-multichain-${timestamp}.json`;
...
export default function () {
  ...
  // Build request - using Target-Service-Id header instead of App-Address
...
function textSummary(data, chainStats) {
  let summary = '\n';
  summary += '╔═══════════════════════════════════════════════════════╗\n';
  summary += '║     Local PATH Gateway Load Test Results (ETH)     ║\n';
  summary += '╚═══════════════════════════════════════════════════════╝\n\n';
