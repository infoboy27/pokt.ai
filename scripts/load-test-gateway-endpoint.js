...
// Tests: https://pokt.ai/api/gateway?endpoint=ethereum_mainnet_1766106468529
// Configurable RPS and total requests
...
const ENDPOINT_ID = __ENV.ENDPOINT_ID || 'ethereum_mainnet_1766106468529';
...
const RPC_METHODS = [
  ...
  { method: 'eth_call', params: [{ to: '0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb', data: '0x70a08231' }, 'latest'] },
];

// Test configuration
const TARGET_RPS = parseInt(__ENV.TARGET_RPS || '100');
const TOTAL_REQUESTS = parseInt(__ENV.TOTAL_REQUESTS || '10000');
...
const methodCounter = {};
RPC_METHODS.forEach(m => {
  methodCounter[m.method] = new Counter(`method_${m.method}`);
});

export const options = {
  stages: [
    // Stage 1: Warm-up (30 seconds) - ramp to 10% of target
    { duration: '30s', target: Math.max(1, Math.floor(TARGET_RPS * 0.1)) },
    
    // Stage 2: Ramp-up to 50% (30 seconds)
    { duration: '30s', target: Math.max(1, Math.floor(TARGET_RPS * 0.5)) },
...
<head>
    ...
    <title>pokt.ai Gateway Load Test Report</title>
    <style>
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="summary">
                <div class="metric-card ${successRate >= 95 ? 'success' : successRate >= 90 ? 'warning' : 'error'}">
                ...
                <div class="metric-card ${avgDuration < 500 ? 'success' : avgDuration < 1000 ? 'warning' : 'error'}">
                ...
                <div class="metric-card ${errorRate < 5 ? 'success' : errorRate < 10 ? 'warning' : 'error'}">
            ...
            <div class="section">
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr><td>Average Response Time</td><td>${(avgDuration / 1000).toFixed(3)}s</td><td><span class="status-badge status-${avgDuration < 500 ? 'success' : avgDuration < 1000 ? 'warning' : 'error'}">${avgDuration < 500 ? 'Excellent' : avgDuration < 1000 ? 'Good' : 'Needs Improvement'}</span></td></tr>
                        <tr><td>P95 Response Time</td><td>${(p95Duration / 1000).toFixed(3)}s</td><td><span class="status-badge status-${p95Duration < 2000 ? 'success' : p95Duration < 5000 ? 'warning' : 'error'}">${p95Duration < 2000 ? 'Excellent' : p95Duration < 5000 ? 'Good' : 'Needs Improvement'}</span></td></tr>
                        <tr><td>P99 Response Time</td><td>${(p99Duration / 1000).toFixed(3)}s</td><td><span class="status-badge status-${p99Duration < 5000 ? 'success' : p99Duration < 10000 ? 'warning' : 'error'}">${p99Duration < 5000 ? 'Excellent' : p99Duration < 10000 ? 'Good' : 'Needs Improvement'}</span></td></tr>
                        <tr><td>Requests per Second</td><td>${avgRPS.toFixed(2)} req/s</td><td><span class="status-badge status-${Math.abs(avgRPS - TARGET_RPS) < TARGET_RPS * 0.1 ? 'success' : 'warning'}">${Math.abs(avgRPS - TARGET_RPS) < TARGET_RPS * 0.1 ? 'On Target' : 'Variance'}</span></td></tr>
                        <tr><td>Error Rate</td><td>${errorRate.toFixed(2)}%</td><td><span class="status-badge status-${errorRate < 5 ? 'success' : errorRate < 10 ? 'warning' : 'error'}">${errorRate < 5 ? 'Excellent' : errorRate < 10 ? 'Acceptable' : 'High'}</span></td></tr>
                    </tbody>
                </table>
            </div>
        </div>
        <div class="footer">
            ...
            <p>Powered by <a href="https://pokt.ai">pokt.ai</a> | Built on <a href="https://www.pokt.network">Pocket Network</a></p>
        </div>
    </div>
</body>
</html>`;
...
export function handleSummary(data) {
  ...
  const summaryFile = `load-test-results/gateway-${ENDPOINT_ID}-${timestamp}.json`;
...
function textSummary(data, methodStats) {
  let summary = '\n';
  summary += '╔═══════════════════════════════════════════════════════╗\n';
  summary += '║     pokt.ai Gateway Load Test Results                ║\n';
  summary += '╚═══════════════════════════════════════════════════════╝\n\n';
